...
if __name__ == "__main__":
    ...
    # 获取 simulation.py 文件所在的目录
    current_dir = os.path.dirname(__file__)

    # 构建目标输出目录的路径
    output_dir = os.path.join(current_dir, "..", f"{scene_name}_output")
    ...
    while window.running:
        ...
        if output_frames:
            if cnt % output_interval == 0:
                os.makedirs(os.path.join(output_dir, f"{cnt:06}"), exist_ok=True)
                window.save_image(os.path.join(output_dir, f"{cnt:06}/raw_view.png"))
                
        if cnt % output_interval == 0:
            if output_ply:
                ply_output_dir = os.path.join(output_dir, f"{cnt:06}")
                ...
                for f_body_id in container.object_id_fluid_body:
                    ...
                    writer.export_ascii(os.path.join(ply_output_dir, f"particle_object_{f_body_id}.ply"))
            if output_obj:
                obj_output_dir = os.path.join(output_dir, f"{cnt:06}")
                os.makedirs(obj_output_dir, exist_ok=True)
                for r_body_id in container.object_id_rigid_body:
                    with open(os.path.join(obj_output_dir, f"mesh_object_{r_body_id}.obj"), "w") as f:
